/**
 * 1.IE的事件流叫冒泡 Netscape的事件流叫事件捕获
 * DOM事件流 三个过程 事件捕获阶段 ,处于目标阶段 和事件冒泡阶段 (即先捕获发生事件的元素 ,然后响应事件冒泡的形式)
 * 
 ...
 */
  ...
  * 
  * DOM2级事件处理程序
  * xx.addEventListener('click',function(){});//接收三个参数 ,最后一个是布尔值,如果是true表示在捕获阶段调用处理函数;false表示在冒泡阶段调用事件处理函数 
  * (建议 不要再事件捕获阶段注册事件处理程序  ,如果第三个参数不传, 默认是冒泡阶段注册)
  ...
  */
  ...
  /**
   * 事件对象 event 可以用event.type 来判断事件的类型 'click'
